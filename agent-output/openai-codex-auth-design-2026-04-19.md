...
## 2026-04-20 Phase A 実装メモ

### 公式 docs 確認

- OpenAI Help Center: `Using Codex with your ChatGPT plan`
  - Codex は ChatGPT Plus / Pro / Business / Enterprise/Edu などのプランに含まれる
  - Codex client から ChatGPT アカウントで sign in する導線が案内されている
- OpenAI Developers: `Codex CLI`
  - Codex CLI 初回起動時に ChatGPT アカウントまたは API key で認証する案内がある

今回の Phase A では、公開 docs から読み取れる「ChatGPT ログイン方式と API key 方式は区別される」という前提だけを使い、
未公開の token exchange 仕様や ChatGPT web cookie / session には触れない。

### What

Task 5007 の最初の PR 範囲として、`OpenAI Codex / ChatGPT ログイン方式` の実 OAuth 連携ではなく、
DB と status API までを実装した。
...
/settings/llm
  -> DELETE /api/llm/providers/openai-codex/disconnect
  -> core usecase
  -> llm_provider_credentials から削除
```

### Why

後続で OAuth connect / callback / token refresh / Agent Gateway 実行を足す前に、
「ワークスペース単位で Codex 系認証情報を持てる場所」と「UI から状態を確認する通り道」を先に固定するため。

また、`wire offline` は API サーバーに届かない状態であり、`expired` / `reauth_required` は保存済み認証情報の問題である。
この2つを UI と API レスポンスで分けた。

### How

- `packages/db/src/schema/llm-provider-credentials.ts`
  - `llm_provider_credentials` テーブルを追加
  - `workspace_id + provider` をユニークにし、1ワークスペース1 provider 接続を基本にした
  - `access_token_encrypted` / `refresh_token_encrypted` として、秘密情報は暗号化済みで保存する責務を明記
- `packages/db/src/repositories/llm-provider-credential-repository.ts`
  - workspace/provider 単位の取得、upsert、disconnect 用削除を追加
- `packages/core/src/usecases/llm-provider-credentials.ts`
  - `missing` / `connected` / `expired` / `reauth_required` に状態を正規化
  - 実 token refresh は未実装
- `apps/api/src/routes/llm.ts`
  - `GET /api/llm/providers/openai-codex/status`
  - `DELETE /api/llm/providers/openai-codex/disconnect`
  - RBAC は `llm:read` / `llm:manage`
- `apps/web/src/components/settings/llm/LlmProviderConnectionPanel.tsx`
  - `/settings/llm` に Codex 接続状態を表示
  - API offline と再認証必要を別の表示にした

### 未対応

- OpenAI / Codex OAuth connect 開始
- callback での token exchange
- token refresh
- `packages/llm/src/adapters/openai-codex.ts`
- Agent Gateway からの `openai-codex` 実行
- ChatGPT web cookie / session を使う非公式実装
