...
done_when:
  ...
  - 5 ページすべての masthead が「英語 kicker → 日本語 h1 → 任意の subheading / tagline」の構造を持つ
...
- 小さな英語 kicker 行を上部に追加（現状 masthead の日付行と同じタイポで `{SECTION_KICKERS.dashboard}`）
- h1 は `MASTHEAD_TITLES.dashboard.ja` = "ダッシュボード"（Fraunces, 大）
- h1 下の小さい italic 副題は `MASTHEAD_TITLES.dashboard.tagline`
...
{/* 既存の italic 副題（Fraunces）として tagline を追加 */}
<p className="... italic">{MASTHEAD_TITLES.posts.tagline}</p>
<p>{meta.total.toLocaleString()} 件の投稿 · ...</p>
```

件数行（動的）は残す。tagline は件数行の上に 1 行追加するだけで OK。

### posts/new

現状 h1 「新しい投稿を作成」を `MASTHEAD_TITLES.postsNew.ja` = "新しい投稿" に置換。kicker は `MASTHEAD_TITLES.postsNew.kickerKey` = `"compose"` 経由で `SECTION_KICKERS.compose` = "Draft Desk" を参照（既存挙動と同値）。tagline を h1 下に追加。

### calendar

現状 h1 「予約を一枚の暦で見渡す」→ `MASTHEAD_TITLES.calendar.ja` = "カレンダー"。
現状 h1 下の日付テキスト `{titleText}`（`2026年 4月` 等）は残す。tagline は日付行の代わりに入れるのではなく、kicker 行の直下 / h1 との間あるいは h1 直下の italic として追加。

### inbox

同様に「会話を一箇所で読む」→ `MASTHEAD_TITLES.inbox.ja` = "受信トレイ"、tagline を追加。
