...
done_when:
  ...
  - 対象 3 ページすべての masthead が「英語 kicker → 日本語 h1」の 2 要素構造（+ 任意の英語 subheading）を持つ
...
- `page.tsx` で `eyebrow`, `title` を `MASTHEAD_TITLES.skills` から生成して渡す
  ```tsx
  ...
  <SkillsMasthead
    ...
    title={m.ja}
    description={m.en}  // 英語 subheading として利用、または description prop を空文字で渡す
    packageCount={...}
  />
  ```
- `SkillsMasthead.tsx` の interface は変更しない（props 名を維持）。内部のレンダリングも変更しない。本質は呼び出し側の文言差し替えのみ
- `description` prop は既存の長い日本語説明文を削除し、英語 subheading (`m.en`) か空にする。既存 UI の空スロットは許容
